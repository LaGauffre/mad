...
$$
\end{solution}
\part[1] Donner la proportion de temps passé par la puce sur chacun des sommets et au centre du carré pour une trajectoire de $(X_n)_{n\in\mathbb{N}}$ de longueur infinie.
\begin{solution}
...
$$
\end{solution}
\question Soient $a,b\geq0$, et $(N,Y)$ un couple de variables aléatoires à valeur dans $\mathbb{N}\times\mathbb{R}_+$ dont la loi est donnée par
$$
\mathbb{P}(N = n, Y\leq t) =b\int_{0}^{t}\frac{(ay)^n}{n!}\exp[-(a+b)y]\text{d}y. 
$$
\begin{parts}
\part[1] Montrer par récurrence sur $n\geq0$ que 
$$
\int_{0}^{\infty}e^{-y}y^{n}\text{d}y = n!
$$
\begin{solution}
Intégration par partie pour l'hérédité. 
\end{solution}
\part[1] Montrer que 
$$
\mathbb{P}(N = n) = \frac{b}{a+b}\left(\frac{a}{a+b}\right)^n.
$$
\begin{solution}
On a 
$$
\mathbb{P}(N = n)=\underset{t\rightarrow\infty}{\lim}\mathbb{P}(N = n, Y\leq t).
$$
\end{solution}
\part[1] Montrer que la densité conditionnelle de $Y|N = n$ s'écrit
$$
f_{Y|N=n}(y) = \frac{(a+b)^{n+1}y^ne^{-(a+b)y}}{n!}
$$
\begin{solution}
On a 
$$
F_{Y|N}(t|n) =\mathbb{P}( Y\leq t|N = n) =\frac{\mathbb{P}( Y\leq t,N = n)}{\mathbb{P}(N = n)} = \int_{0}^t\frac{(a+b)^{n+1}y^ne^{-(a+b)y}}{n!}\text{d}y 
$$
puis
$$
f_{Y|N=n}(y)  =\frac{\partial}{\partial y}F_{Y|N}(t|n).
$$
\end{solution}
\part[1] Calculer $\mathbb{E}(Y|N = n)$
\begin{solution}
$\mathbb{E}(Y|N = n) = \frac{n+1}{a+b}$
\end{solution}
\part[1] Calculer $\mathbb{E}\left(\frac{Y}{N+1}\right)$ 
\begin{solution}
\begin{eqnarray*}
\mathbb{E}\left(\frac{Y}{N+1}\right) &=&  \mathbb{E}\left[\mathbb{E}\left(\frac{Y}{N+1}|N\right)\right]\\
&=&\sum_{n=0}^\infty \mathbb{E}\left(\frac{Y}{N+1}|N=n\right)\mathbb{P}(N=n)\\
&=&\sum_{n=0}^\infty\frac{1}{n+1}\frac{n+1}{a+b}\frac{b}{a+b}\left(\frac{a}{a+b}\right)^n\\
&=&\frac{1}{a+b}
\end{eqnarray*}
\end{solution}
\end{parts}
